...
function initApp() {
    ...
    navReports.addEventListener('click', (e) => {
        e.preventDefault();
        exportToExcel();
        // Ensure production page is hidden when showing reports
        productionPage.classList.add('hidden'); });
...
// Add loadProductionOrders function
function loadProductionOrders() {
    // Show loading state
    const loadingIndicator = document.createElement('div');
    loadingIndicator.className = 'loading-spinner';
    productionPage.querySelector('.production-container').appendChild(loadingIndicator);
    
    // Get all orders from Firebase, ordered by timestamp
    database.ref('orders').orderByChild('timestamp').once('value')
        .then((snapshot) => {
            const newOrdersTable = document.getElementById('new-orders-table').getElementsByTagName('tbody')[0];
            const processingTable = document.getElementById('processing-orders-table').getElementsByTagName('tbody')[0];
            const completedTable = document.getElementById('completed-orders-table').getElementsByTagName('tbody')[0];
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                const rowData = {
                    ...
                    type: order.sheetType ? order.sheetType.charAt(0).toUpperCase() + order.sheetType.slice(1) : 'N/A',
                    length: `${order.length}m`,
                    qty: order.quantity,
                    startedAt: formatDateTime(order.processingStarted),
                    completedAt: formatDateTime(order.completedAt)
                };
                
                // Determine which table to add to based on status
                const status = order.status || ORDER_STATUS.NEW;
                
                if (status === ORDER_STATUS.NEW) {
                    const row = newOrdersTable.insertRow();
                    row.innerHTML = `
                        <td>${rowData.id.substring(0, 8)}...</td>
                        <td>${rowData.date}</td>
                        ...
                        <td>${rowData.qty}</td>
                        <td>
                            <button class="status-btn accept" onclick="updateOrderStatus('${orderId}', '${ORDER_STATUS.PROCESSING}')">
                                <i class="fas fa-check"></i> Accept
                            </button>
                        </td>
                    `;
                } else if (status === ORDER_STATUS.PROCESSING) {
                    const row = processingTable.insertRow();
                    row.innerHTML = `
                        <td>${rowData.id.substring(0, 8)}...</td>
                        <td>${rowData.date}</td>
                        <td>${rowData.customer}</td>
                        ...
                        <td>${rowData.length}</td>
                        <td>${rowData.qty}</td>
                        <td>${rowData.startedAt}</td>
                        <td>
                            <button class="status-btn complete" onclick="updateOrderStatus('${orderId}', '${ORDER_STATUS.COMPLETED}')">
        ...
        .catch((error) => {
            ...
            showAlert("Failed to load production orders", "error");
        })
        .finally(() => {
            // Remove loading indicator
            const container = productionPage.querySelector('.production-container');
...
async function updateOrderStatus(orderId, newStatus) {
    try {
        ...
        if (newStatus === ORDER_STATUS.PROCESSING) {
            updates.processingStarted = now;
            updates.processingStartedTimestamp = Date.now(); // Store timestamp as number for sorting
        } else if (newStatus === ORDER_STATUS.COMPLETED) {
            updates.completedAt = now;
            updates.completedAtTimestamp = Date.now(); // Store timestamp as number for sorting
        }
